...
Preserve Fonte e Foto em linhas separadas e os créditos reais. Não troque a fonte pelo nome da página. Preserve as hashtags do original. Se houver chamada para seguir, adapte somente essa chamada ao nome do destino. Não acrescente uma segunda manchete ao corpo.` },
    { role: 'user', content: JSON.stringify({ pagina: page.page_name, principal: {
      titulo: source.titulo, materia: source.materia, hashtags: source.hashtags,
      fonte: source.fonte_credito, url: source.fonte_url,
    }, versoesAnteriores: previous.map((p) => ({ titulo: p.titulo, materia: p.materia })) }) },
  ];
  ...
  const check = await chatCompletionClaudeObrigatorio([
    ...
    { role: 'user', content: JSON.stringify({ principal: source.materia, tituloPrincipal: source.titulo, versao: variation }) },
...
module.exports = { parseBrand, effectiveBrand, fingerprint, normalized, validateVariation, generateVariation };
